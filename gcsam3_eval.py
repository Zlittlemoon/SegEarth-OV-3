...
def main():
    ...
    # [修改 2] 这一步很关键！
    # 你需要通过 cfg-options 或者在这里强制修改配置，把训练好的权重路径传进去
    # 假设你现在的 config 文件里 model 字段没有写 adapter_ckpt_path
    
    # 强制覆盖模型配置
    # 注意：这里的路径要改成你实际训练出来的权重路径
    ...
    if runner.rank == 0:
        append_experiment_result('results_train_on_potsdam.xlsx', [results])

    if runner.rank == 0:
        with open(os.path.join(cfg.work_dir, 'results_train_on_potsdam.txt'), 'a') as f:
