...
"""
Check and visualize SAM3 semantic JSON by class.

Compared with the original checker:
- sample visualization PER CLASS instead of global random sampling
- save visualizations into class-specific folders
- easier to diagnose "query text does not match mask" problems

Example:
python tools/check_sam3_semantic_json_by_class.py \
  --json data/sam3_semantic_fixed/isaid/train.sam3.semantic.json \
  --img-root data/iSAID_split/train/images \
  --visualize-per-class 10 \
  --vis-out outputs/json_check_vis_by_class/isaid_train

python tools/check_sam3_semantic_json_by_class.py \
  --json data/sam3_semantic_fixed/isaid/val.sam3.semantic.json \
  --img-root data/iSAID_split/val/images \
  --visualize-per-class 10 \
  --vis-out outputs/json_check_vis_by_class/isaid_val
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Check SAM3 semantic JSON by class")
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of samples to visualize per class (0 disables visualization)",
    )
    parser.add_argument(
        "--only-classes",
        nargs="*",
        default=None,
        help="Optional whitelist of class names to visualize, e.g. ship harbor plane",
    )
    parser.add_argument(
        "--vis-out",
        type=Path,
        default=Path("outputs/json_check_vis_by_class"),
        help="Output directory for visualization images",
    )
    parser.add_argument("--seed", type=int, default=123, help="Random seed")
    parser.add_argument("--max-print-errors", type=int, default=50)
...
def main() -> None:
    ...
    print("========== SAM3 Semantic JSON Check (By Class) ==========")
    ...
    _ = build_id_index(queries, "queries") if queries else {}
    ...
    for q in queries:
        ...
        if ok:
            ...
            queries_by_text[query_text].append(q)
    ...
    if args.visualize_per_class > 0:
        print("[5/5] Writing visualization samples by class...")
        args.vis_out.mkdir(parents=True, exist_ok=True)

        allowed_classes = None
        if args.only_classes:
            allowed_classes = set(args.only_classes)

        class_names = sorted(queries_by_text.keys())
        for class_name in class_names:
            if allowed_classes is not None and class_name not in allowed_classes:
                continue

            class_queries = queries_by_text[class_name]
            if len(class_queries) == 0:
                continue

            n_vis = min(args.visualize_per_class, len(class_queries))
            sampled = random.sample(class_queries, n_vis)

            class_dir = args.vis_out / slugify(class_name)
            class_dir.mkdir(parents=True, exist_ok=True)

            print(f"  class={class_name} total={len(class_queries)} visualize={n_vis}")
            ...
            for i, q in enumerate(sampled):
                ...
                out_path = class_dir / (
                    f"{i:04d}_{slugify(class_name)}_img{image_id}_q{q['id']}.png"
                )
                vis.save(out_path)
